...
import type { Rating, ReviewRecord, ReviewState } from '../types.js';
import type { ImportOptions, ImportResult } from './converter.js';
import type { AnkiRevlog } from './schema.js';
import { appendFileSync, existsSync, mkdirSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import { convertAnkiToIR } from './converter.js';
...
export { type ImportedNote, type ImportOptions, type ImportResult } from './converter.js';
...
function buildSidecarContent(note: {
	...
	if (
		note.type === 'item' &&
		note.cards.some((c) => c.clozeIndex !== undefined && c.clozeIndex !== 0)
	) {
		data.cloze = note.cards
			.filter((c) => c.clozeIndex !== undefined && c.clozeIndex !== 0)
			.map((c) => `c${c.clozeIndex}`);

		const clozes: Record<string, unknown> = {};
		for (const card of note.cards) {
			if (card.clozeIndex === undefined || card.clozeIndex === 0) continue;
...
function serializeState(state: ReviewState): Record<string, unknown> {
	return {
		status: state.status,
