...
import type { NotePlatform, Rating, SessionConfig, SessionStrategyId } from '../types';
...
export async function runRvCommands(
	...
	for (const command of commands) {
		if (command.name === 'expect') {
			...
			if (path === undefined || path === '' || expectedRaw === '') {
	...
	switch (command.name) {
		case 'topic': {
			...
			const noteId = await store.createNote(content, {
				title: options.title,
				priority:
					options.priority !== undefined && options.priority !== ''
		...
		case 'session': {
			...
			if (positional[0] !== undefined && positional[0] !== '')
				newConfig.strategy = positional[0] as SessionStrategyId;
			if (options.exam !== undefined && options.exam !== '')
				newConfig.examDate = new Date(options.exam);
			if (options.capacity !== undefined && options.capacity !== '')
				newConfig.capacity = Number(options.capacity);
			if (options.clump !== undefined && options.clump !== '')
				newConfig.clumpLimit = Number(options.clump);
			if (options.cooldown !== undefined && options.cooldown !== '')
				newConfig.cooldown = Number(options.cooldown);
			if (options['new-cards-limit'] !== undefined && options['new-cards-limit'] !== '')
