import type { Editor as IEditor } from 'obsidian'
import { App } from 'obsidian'
import { describe, expect, it } from 'vitest'
import { Editor, Notice } from '../../tests/obsidian-stub'
import { clozeSelection, clozeSelectionNextIndex, clozeSelectionSameIndex } from '../cloze'
...
describe('cloze selection', () => {
	it('wraps selection in cloze markup', async () => {
		...
		await clozeSelection(app, editor as unknown as IEditor, null, { index: 2 })
	...
	it('uses the next available index', async () => {
		...
		await clozeSelectionNextIndex(app, editor as unknown as IEditor, null, {})
		expect(editor.getValue()).toBe('{{c2::delta}} {{c1::beta}}')
	})

	it('reuses the last cloze index', async () => {
		const editor = new Editor('first')
		await clozeSelection(app, editor as unknown as IEditor, null, { index: 3 })
		editor.setValue('second')
		await clozeSelectionSameIndex(app, editor as unknown as IEditor, null, {})
	...
	it('shows a notice when selection is empty', async () => {
		...
		await clozeSelection(app, editor as unknown as IEditor, null, {})
	...
	it('updates frontmatter and sidecar when file is provided', async () => {
		...
		const editor = new Editor('Alpha')
		await clozeSelection(app, editor as unknown as IEditor, note, {
			index: 1,
			extractTag: 'topic',
		})
