import type { App as IApp, Editor as IEditor, MarkdownView as IMarkdownView } from 'obsidian'
import type { Vault } from '../../tests/obsidian-stub'
import { formatDate } from '@repo/core/core/frontmatter'
import { TFile } from 'obsidian'
import { describe, expect, it } from 'vitest'
import { App, Editor, MarkdownView, Notice } from '../../tests/obsidian-stub'
import {
...
function getFrontmatter(app: IApp, path: string): Record<string, unknown> {
...
describe('vault stub edge cases', () => {
	...
	it('getFileCache handles content without frontmatter', () => {
		const app = new App()
		;(app.vault as unknown as Vault).setContent('test.md', 'No frontmatter here')
...
describe('extractToIncrementalNote', () => {
	it('shows notices when selection or file is missing', async () => {
		...
		await extractToIncrementalNote(
			app as unknown as IApp,
			emptyEditor as unknown as IEditor,
			view as unknown as IMarkdownView,
			{ titleWords: 2 },
		)
	...
	it('creates a child note, link, and sidecar', async () => {
		...
		const frontmatter = getFrontmatter(app as unknown as IApp, childPath)
		...
		expect(Notice.messages.some((msg: string) => msg.includes('Created note: Line one'))).toBe(
			true,
		)
	...
	it('optionally converts source note to folder note', async () => {
		...
		const frontmatter = getFrontmatter(app as unknown as IApp, 'Folder/Source Note/Line one.md')
	...
	it('does not convert when source already is a folder note', async () => {
		const app = new App()
		const source = await app.vault.create('Folder/Source Note/Source Note.md', 'Source content')
		const view = new MarkdownView(source)
		const selection = 'Line one\r\nLine two'
		const editor = new Editor(selection)
		Notice.clear()
	...
	it('skips conversion when a folder note with same name already exists', async () => {
		...
		const frontmatter = getFrontmatter(app as unknown as IApp, 'Folder/Line one.md')
		expect(frontmatter.source).toBe('[[Folder/Source Note]]')
