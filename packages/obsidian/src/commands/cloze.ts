...
import { getHighestClozeIndex, getNextClozeIndex } from '@repo/core/core/cloze'
import { normalizeTags } from '@repo/core/core/frontmatter'
...
export async function clozeSelection(
	...
	if (file) {
		const tag =
			options.extractTag !== undefined && options.extractTag.trim() !== ''
				? options.extractTag.trim()
				: 'topic'
