...
import type { ButtonComponent, SliderComponent, TextComponent } from './obsidian-stub';
import { describe, expect, it } from 'vitest';
import { PriorityModal } from '../PriorityModal';
import { App, Setting } from './obsidian-stub';
...
describe('priorityModal', () => {
	it('syncs slider and text inputs and clamps on submit', () => {
		...
		const modal = new PriorityModal(app as unknown as IApp, 20, (value) => {
			submitted = value;
		});
	...
	it('empties content on close', () => {
		const app = new App();
		const modal = new PriorityModal(app as unknown as IApp, 10, () => undefined);
		modal.open();
