...
function buildDecorations(view: EditorView, state: ClozeState): DecorationSet {
	if (state.index === null || state.index === 0 || state.phase !== 'question') {
		return Decoration.none
	}
	const doc = view.state.doc.toString()
	const decorations: { from: number; to: number; deco: Decoration }[] = []
	// Use simpler regex that avoids backtracking issues
	// Non-capturing group for answer content since we only need index and hint
	const regex = /\{\{c(\d+)::[^:}]+(?::[^:}]+)*(?:::([^}]+))?\}\}/g
	for (const match of doc.matchAll(regex)) {
		const index = Number(match[1])
		if (index !== state.index) continue
		const hint = match[2]
		const placeholder = hint !== undefined && hint !== '' ? `[...] (${hint})` : '[...]'
