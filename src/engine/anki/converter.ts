...
import type { AnkiData, AnkiCard } from './schema.js';
import { ANKI_QUEUE, ANKI_MODEL_TYPE } from './schema.js';
import { filterDeckIds } from './reader.js';
import { parseAnkiFields, fieldsToMarkdown } from './html.js';
...
export function convertAnkiToIR(data: AnkiData, options: ImportOptions = {}): ImportResult {
	...
	for (const note of data.notes) {
		...
		for (const card of cards) {
			...
			const irItemId = clozeUid ? `${noteId}::${clozeUid}` : noteId;
		...
		const content = buildNoteContent({
			...
			tag,
			priority: 50,
			clozeIndices: isCloze ? importedCards.map((c) => c.clozeIndex!).filter(Boolean) : [],
		});
