...
export class MarkdownDataStore implements DataStore {
		...
		for (const path of itemFiles) {
			...
			if (data.clozes) {
				...
				for (const key of Object.keys(data.clozes)) {
					const clozeIndex = parseInt(key.substring(1), 10);
					...
					items.push({
						...
						notePath: data.note_path,
						type: 'cloze',
						clozeIndex: isNaN(clozeIndex) ? null : clozeIndex,
	...
	async getState(itemId: string): Promise<ReviewState | null> {
		...
		if (clozeKey) {
			if (data.clozes && data.clozes[clozeKey]) {
				const entry = data.clozes[clozeKey];
				if (!entry) return null;
				// eslint-disable-next-line @typescript-eslint/no-unused-vars
				const { cloze_uid: _, ...state } = entry;
				return state;
			}
			return null;
		} else {
...
${line}`
	...
	private parseState(raw?: RawItemState): ReviewState | undefined {
		...
		return {
			...
			reps: raw.reps ?? 0,
			lapses: raw.lapses ?? 0,
			lastReview: raw.last_review ? new Date(raw.last_review) : null,
		};
	}
